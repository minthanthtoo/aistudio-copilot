...
3. Describe a small app. Confirm the Draft Plan appears immediately, asks no more than three questions, and separates **You chose**, **Suggested**, **Still needed**, and **Needs review**.
4. Return to Build and choose **Full Wizard** in the visible workflow chooser. Confirm the original step-by-step form opens with the description preserved; then return and choose Draft Plan again.
5. Change or skip one Draft Plan question, reload, and confirm the Draft Plan, active question, and provenance rehydrate without duplicate roots or listeners.
6. Approve assumptions to **Add to Queue** and confirm exactly one chain is added with no host click. Repeat with **Approve assumptions & Run** and confirm exactly one chain and one host submission.
7. Load a built-in or saved template, confirm its values are proposed, and verify **Save as Template** persists one entry.
8. In one disposable app, paste A, B, and C without leaving Build. Use **Single prompt** for B, then return to **Auto-detect** for C. Confirm the stack meter advances to `3 chain(s) · 5 prompt(s)`.
9. In Prompts, rename the chains A, B, and C. Move C before B and confirm order A, C, B.
10. Start the full stack. While A1 is running, inspect B and confirm the Run tab still identifies A/A1 as runner-owned.
11. After C1 visibly starts, reload the AI Studio page once. Confirm the pending marker does not submit twice and execution safely resumes or pauses with an explicit diagnostic. In a different app tab, confirm Pause/Skip are unavailable and **Recover here** rejects the wrong app.
12. At completion, verify the user-turn/transcript marker order is exactly `AISQ_A1 → AISQ_A2 → AISQ_C1 → AISQ_C2 → AISQ_B1`.
13. Verify the preview list is exactly `AISQ_A1_OK`, `AISQ_A2_OK`, `AISQ_C1_OK`, `AISQ_C2_OK`, `AISQ_B1_OK`.
14. Download redacted diagnostics and confirm it contains statuses and lengths but none of the fixture prompt text.
15. Run **Download ZIP** and verify Code → Export options → Download as `.zip` initiates the application archive.
16. Record screenshots, transcript observations, reload behavior, diagnostics result, console errors, and ZIP result in `docs/live-verification.md`.
