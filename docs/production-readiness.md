...
| Regression reliability | pass | `npm run verify` passes all 132 tests, including manifest-order integration, both workflow paths, Draft Plan flows, lease fencing, migration, reinjection, queue logic, and the fetch-policy allowlist | Any later runtime change invalidates this pass | Re-run after any live-driven code repair |
