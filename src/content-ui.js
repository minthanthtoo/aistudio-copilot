(function initAISQContentUI(global) {
  ...
  function render() {
    ...
    const header = ctx.el("header", { className: "aisq-header" }, [
      ...
      ctx.el("div", { className: "aisq-window-controls" }, [
        ...
        ctx.button(ctx.state.settings.isMinimized ? "◱" : "—", () => mutate(() => { ctx.state.settings.isMinimized = !ctx.state.settings.isMinimized; }), "icon", ctx.state.settings.isMinimized ? "Maximize Copilot" : "Minimize Copilot"),
        ctx.button("×", () => mutate(() => { ctx.state.settings.panelOpen = false; }), "icon", "Close Copilot")
      ])
    ]);
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      * { box-sizing: border-box; }
      ...
      #aisq-bubble { position:fixed; right:18px; bottom:18px; z-index:2147483647; width:50px; height:50px; border:1px solid #ffffff33; border-radius:18px; background:linear-gradient(145deg,#7357ff,#4b2acb); color:#fff; font:800 13px/1 system-ui,sans-serif; box-ctx.shadow:0 14px 40px #0008; cursor:pointer; }
      #aisq-bubble.running::after { content:""; position:absolute; right:5px; top:5px; width:9px; height:9px; border-radius:50%; background:#55e69b; box-ctx.shadow:0 0 0 3px #173c2c; }
      #aisq-dl-bubble { position:fixed; right:74px; bottom:18px; z-index:2147483647; width:50px; height:50px; border:1px solid #ffffff33; border-radius:18px; background:linear-gradient(145deg,#333,#111); color:#fff; font:800 20px/1 system-ui,sans-serif; box-ctx.shadow:0 14px 40px #0008; cursor:pointer; display:none; }
      #aisq-dl-bubble:hover { background:linear-gradient(145deg,#444,#222); }
      .aisq-badge { position:fixed; z-index:2147483647; padding:6px 10px; border-radius:999px; font:12px/1.2 system-ui,-apple-system,sans-serif; background:#000d; color:#fff; box-ctx.shadow:0 8px 24px #0004; pointer-events:none; }
      #aisq-ctx.panel { position:fixed; right:18px; bottom:80px; z-index:2147483647; width:min(500px,calc(100vw - 24px)); max-height:min(760px,calc(100vh - 100px)); overflow:auto; border:1px solid #ffffff26; border-radius:18px; background:#15151a; color:#f5f4fa; font:13px/1.45 system-ui,-apple-system,sans-serif; box-ctx.shadow:0 24px 80px #000b; }
      #aisq-ctx.panel[hidden] { display:none; }
      ...
      .aisq-top-run-controls { position:sticky; top:99px; z-index:2; display:flex; gap:8px; padding:10px 16px; background:#1e1d24f2; backdrop-filter:blur(12px); border-bottom:1px solid #ffffff12; align-items:center; flex-wrap:wrap; box-ctx.shadow:0 4px 14px rgba(0,0,0,0.35); border-radius:0 0 16px 16px; }
      ...
      .aisq-btn-highlight { background:linear-gradient(135deg,#7357ff,#5835eb) !important; border-color:#8e76ff !important; color:#fff !important; box-ctx.shadow:0 2px 8px rgba(115,87,255,0.35); font-weight:600; }
      ...
      .aisq-host-editor::before { background:#55e69b; box-ctx.shadow:0 0 6px #55e69b; }
      ...
      .aisq-host-busy::before { background:#b9a9ff; box-ctx.shadow:0 0 8px #b9a9ff; }
      #aisq-ctx.panel.aisq-minimized { width: auto; max-width: 480px; min-width: 320px; bottom: 80px; }
      #aisq-ctx.panel.aisq-minimized .aisq-tabs,
      #aisq-ctx.panel.aisq-minimized .aisq-global-error,
      #aisq-ctx.panel.aisq-minimized .aisq-ctx.panel > div:not(.aisq-top-run-controls),
      #aisq-ctx.panel.aisq-minimized .aisq-scroll-actions,
      #aisq-ctx.panel.aisq-minimized .aisq-footer,
      #aisq-ctx.panel.aisq-minimized .aisq-section { display: none !important; }
      #aisq-ctx.panel.aisq-minimized .aisq-top-run-controls { border-bottom: none; border-radius: 0 0 18px 18px; top: 61px; }
      ...
      .aisq-input:focus,.aisq-draft:focus,.aisq-prompt-editor:focus,.aisq-select:focus { border-color:#8067ff; box-ctx.shadow:0 0 0 3px #7357ff30; }
      ...
      .aisq-quickstart-card { display: flex; flex-direction: column; align-items: flex-start; border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; background: #1d1c22; cursor: pointer; text-align: left; transition: all 0.2s ease; box-ctx.shadow: 0 4px 12px rgba(0,0,0,0.1); }
      .aisq-quickstart-card:hover { transform: translateY(-2px); border-color: rgba(115,87,255,0.4); background: #26252e; box-ctx.shadow: 0 6px 16px rgba(115,87,255,0.15); }
      ...
      .aisq-chain-card { border:1px solid #ffffff16; border-radius:11px; background:#1d1c22; padding:8px; transition:border-color 0.2s, box-ctx.shadow 0.2s; }
      .aisq-chain-card.selected { border-color:#8067ff; box-ctx.shadow:0 0 0 2px #7357ff24; }
      .aisq-chain-card.locked { border-color:#a88cff; }
      .aisq-chain-card.aisq-highlight-running { border-color:#55e69b; box-ctx.shadow:0 0 0 2px rgba(85,230,155,0.2); }
      .aisq-chain-card.aisq-highlight-paused { border-color:#f6c032; box-ctx.shadow:0 0 0 2px rgba(246,192,50,0.2); }
      ...
      .aisq-index { flex:0 0 24px; display:grid; place-items:center; height:24px; border-radius:7px; background:#302d39; user-select:none; cursor:pointer; transition:background 0.15s, transform 0.1s, box-ctx.shadow 0.15s; }
      .aisq-index:hover { background:#4f4863; color:#fff; transform:scale(1.1); box-ctx.shadow:0 0 8px rgba(115,87,255,0.4); }
      ...
      .aisq-dialog { background:#15151a; color:#f5f4fa; border:1px solid #ffffff26; border-radius:12px; padding:20px; box-ctx.shadow:0 24px 80px #000b; max-width:400px; }
      ...
      .aisq-float-btn { width:32px; height:32px; border-radius:50%; border:1px solid #ffffff20; background:rgba(21,21,26,0.9); color:#cfcbd9; cursor:pointer; box-ctx.shadow:0 4px 12px rgba(0,0,0,0.5); backdrop-filter:blur(4px); font-size:12px; display:grid; place-items:center; transition:background 0.2s, border-color 0.2s; }
      .aisq-float-btn:hover { background:rgba(45,42,58,0.9); border-color:#7357ff; }
      .aisq-global-tooltip { position:fixed; z-index:2147483647; background:rgba(20,20,25,0.95); color:#fff; padding:6px 10px; border-radius:6px; font-size:11px; line-height:1.4; pointer-events:none; opacity:0; transition:opacity 0.15s; white-space:normal; max-width:240px; text-align:center; border:1px solid rgba(255,255,255,0.1); box-ctx.shadow:0 4px 12px rgba(0,0,0,0.5); transform:translate(-50%, -100%); margin-top:-8px; }
  ...
  function mount() {
    ...
    const bubble = ctx.el("button", { type: "button", html: bubbleHtml, title: "Toggle AI Studio Copilot", ariaLabel: "Toggle AI Studio Copilot", on: { click: () => mutate(() => { ctx.state.settings.panelOpen = !ctx.state.settings.panelOpen; }) } });
