(function initAISQContentUI(global) {
  ...
  function mount() {
    ...
    sparkle.setAttribute("width", "26");
    sparkle.setAttribute("height", "26");
    sparkle.setAttribute("viewBox", "0 0 24 24");
    sparkle.setAttribute("fill", "currentColor");
    sparkle.style.display = "block";
    sparkle.style.margin = "auto";
    const sparkPath = document.createElementNS(svgNs, "path");
    sparkPath.setAttribute("d", "M12 2C12 7.5 16.5 12 22 12C16.5 12 12 16.5 12 22C12 16.5 7.5 12 2 12C7.5 12 12 7.5 12 2Z");
    sparkle.append(sparkPath);
    const bubble = ctx.el("button", { type: "button", title: "Toggle AI Studio Copilot", ariaLabel: "Toggle AI Studio Copilot", on: { click: () => ctx.mutate(() => { ctx.state.settings.panelOpen = !ctx.state.settings.panelOpen; }) } }, [sparkle]);
