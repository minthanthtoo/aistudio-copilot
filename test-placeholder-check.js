...
const strategies = ["auto", "stage", "id", "prompt", "delimiter", "numbered", "single"];
for (const s of strategies) {
  proxyState.ui = { draft: "", buildView: "input", splitStrategy: s, settings: { activeTab: "build" } };
  const ui = ctx.renderBuild();
  const textarea = ui.querySelector("textarea");
  console.log(`Strategy: ${s.padEnd(10)} -> Placeholder: ${textarea.placeholder.split('\\n')[0]}`);
}
